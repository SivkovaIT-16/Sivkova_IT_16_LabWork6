...
### Алгоритм решения
Разработан тестовый класс TestInvoke для проверки корректности работы с аннотацией @Invoke. Класс использует фреймворк JUnit для организации модульного тестирования. Тесты проверяют работу аннотации и обработки методов через Reflection API.

Перед выполнением каждого теста метод setUp(), помеченный аннотацией @BeforeEach, создаёт новый экземпляр класса Counter (некоторые методы класса помечены аннотацией @Invoke). Это позволяет каждому тесту работать с изолированным экземпляром класса, что предотвращает влияние одного теста на результаты другого.
...
### Алгоритм решения
Разработан тестовый класс TestTwo для проверки обработки аннотации @Two с некорректными значениями свойств. Класс использует фреймворк JUnit для организации модульного тестирования и проверяет корректность работы с аннотацией при ошибочных значениях.

Создан внутренний класс InvalidTwoClass, который помечен аннотацией @Two с намеренно некорректными значениями: first = "" - пустая строка (граничный случай), second = -1 - отрицательное число (нестандартное значение). Этот класс используется для проверки обработки некорректных данных аннотации.

Тест testReadInvalidTwoAnnotationValues(): Чтение некорректных значений аннотации - проверяет, что Reflection API корректно читает некорректные значения из аннотации @Two. Сначала создаёт экземпляр вспомогательного класса InvalidTwoClass и получает аннотацию @Two с класса через getAnnotation(Two.class). Если аннотация отсутствует - выбрасывает AssertionError с описанием ошибки. Далее проверяются значения свойств: если first НЕ равно пустой строке "" или second НЕ равно -1 - выбрасывает AssertionError с описанием ошибки. Успешное выполнение означает, что Reflection API корректно читает любые значения аннотации.

Тест testInvalidTwoAnnotationThrowsException(): Проверка выбрасывания исключения - проверяет, что метод TwoReflection.showTwo() корректно обрабатывает некорректные значения аннотации и выбрасывает соответствующее исключение. Сначала создаёт экземпляр вспомогательного класса InvalidTwoClass. Далее использует метод assertThrows() из JUnit для проверки исключения, в параметры которого передаёт: ожидаемый тип исключения (IllegalArgumentException.class), лямбда-выражение с вызовом TwoReflection.showTwo(invalidObject), соответсвующее сообщение об ошибке при неудачном прохождении теста. Метод assertThrows() автоматически выполняет переданный код и проверяет, что было выброшено указанное исключение. Если исключение не выброшено или выброшено исключение другого типа - тест не пройден. Успешное выполнение означает, что метод showTwo() корректно обрабатывает некорректные данные.

#### Тестирование
При успешном прохождении тестов в консоли не появляются сообщения об ошибках. Тесты проходят "зелёным" цветом в среде выполнения JUnit.
